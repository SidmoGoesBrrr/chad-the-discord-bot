...
class Fun(commands.Cog):
    ...
    async def ask(self, ctx, *, question):
        ...
        list = ["will", "how", "why", "is",
                "when", "where", "who", "whom", "I", "@", "can", "am", "should", "are", "were", "if", "did", "does",
                "do",
                "has", "was"]
    ...
    async def gif(self, ctx, *, q="random"):
        ...
        try:
            ...
            emb = discord.Embed(title=q, description="Sorry could not find anything matching that",
    ...
    async def poll(self, ctx, duration: TimeConverter, *, argument: str):
        if duration != 0:
            ...
            if ':' in argument:
                ...
                await ctx.send(embed=discord.Embed(title="You must put a colon(:) after your poll question.",
                                                   color=discord.Color.random()))
                return
            ...
            if ',' in split_question[1]:
                ...
                await ctx.send(embed=discord.Embed(title="You must put a comma(,) after each poll options.",
                                                   color=discord.Color.random()))
                return
    ...
    async def emojify(self, ctx, *, text: str):
        ...
        if len(emojified) + 2 >= 2000:
            await ctx.send(embed=discord.Embed(title='Your message in emojis exceeds 2000 characters!',
                                               color=discord.Color.random()))
            return
        if len(emojified) <= 25:
            await ctx.send(embed=discord.Embed(title='Your message could not be converted!'),
                           color=discord.Color.random())
            return
        else:
    ...
    async def spoilify(self, ctx, *, text: str):
        ...
        if len(spoilified) + 2 >= 2000:
            await ctx.send(embed=discord.Embed(title='Your message in spoilers exceeds 2000 characters!'),
                           color=discord.Color.random())
            return
        if len(spoilified) <= 4:
            await ctx.send(embed=discord.Embed(title='Your message could not be converted!'),
    ...
    async def act(self, ctx, member: discord.User, *, message=None):
        ...
        if message == None:
            await ctx.send(
                embed=discord.Embed(title=f'Please provide a message with that!', color=discord.Color.random()))
            return

        if '@' in str(message):
            await ctx.send(
                embed=discord.Embed(title=f"Imagine exploiting the act command like dis...\nNo pinging anyone please",
                                    color=discord.Color.random()))
            return
    ...
    async def vcmeme_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await ctx.send(embed=discord.Embed(title="Sorry im already playing audio. Do !stop to stop it",
                                               color=discord.Color.random()))
    ...
    async def shoo(self, ctx):
        try:
            ...
            await ctx.send(
                embed=discord.Embed(title="You are not connected to a voice channel", color=discord.Color.random()
                                    ))
