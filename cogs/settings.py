...
class settings(commands.Cog):
    ...
    async def pingsettings(self, ctx, condition: str):
        ...
        if condition.lower() == "true":
            if checkping(guild_id_var) == 'true':
                ...
                await ctx.send(embed=discord.Embed(title="Alright, I now have permission!",
                                                   description="I shall ping you as and when it is required.",
            ...
            if checkping(guild_id_var) == 'false':
                ...
                await ctx.send(embed=discord.Embed(title="Alright, I don't have permission anymore",
                                                   description="I will not ping you again.",
                                                   color=discord.Color.random()))
                try:
            ...
            await ctx.send(embed=discord.Embed(title="Shoo silly child", description="Invalid option",
                                               color=discord.Color.random()))
    ...
    async def votereminder(self, ctx):
        if ctx.guild.id != 869173101131337748:
            await ctx.send(embed=discord.Embed(title=f"{ctx.author.name} You are an idiot. ",
        ...
        await ctx.send(
            embed=discord.Embed(title=f"Ok {ctx.author.name}. I shall remind you to vote for me every 12 hours!",
                                description="Forgetful boi", color=discord.Color.random()))
