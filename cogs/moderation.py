...
class Moderation(commands.Cog):
    ...
    async def unblacklist(self, ctx, member: discord.Member):
        ...
        if ctx.author.guild_permissions.administrator or ctx.author.id == 815555652780294175 or \
            ...
            if not member:
                embed = discord.Embed(title="Please provide-", description="a member to unblacklist!",
                                      color=discord.Color.random())
        ...
        if ctx.author != member:
            if ctx.author.guild_permissions.administrator:
                if member.guild_permissions.administrator:
                    ...
                    await ctx.send(
                        embed=discord.Embed(title=f"{member.display_name} has been warned", description=reason,
                                            color=discord.Color.random()))
    ...
    async def lockdown(self, ctx, state):
        ...
        if ctx.author.guild_permissions.administrator:
            ...
            if state.lower() == 'true':
                ...
                embed = discord.Embed(title=f"Please give a valid state, True or false",
                                      description=f"Try `{ctx.prefix}lockdown true` or `{ctx.prefix}lockdown false`",
                                      color=discord.Color.random())
                await ctx.send(embed=embed)
        else:
            await ctx.send(
    ...
    async def unmute(self, ctx, member: discord.Member):
        if ctx.author.guild_permissions.manage_messages:
            ...
            if mutedRole in member.roles:
                embed = discord.Embed(title=f"{member.display_name} has now been unmuted!!",
                                      color=discord.Color.blurple())
                embed.set_footer(text="Rejoice son, don't make this mistake again")
                await member.remove_roles(mutedRole)
                await ctx.send(embed=embed)
            else:
                embed = discord.Embed(title="This user isn't even muted.",
    ...
    async def tempmute(self, ctx, duration: TimeConverter, member: discord.Member, *, reason=None):
        ...
        if duration != 0:
            ...
            if mutedRole in member.roles:
                embed = discord.Embed(title="Already muted idiot",
    ...
    async def ban(self, ctx, member: discord.Member, *, reason=None):
        if ctx.author.guild_permissions.ban_members:
            ...
            if reason is None:
                reason = "No reason specified"
            message = discord.Embed(title=f"You have been banned from {ctx.guild.name} for {reason}",
                                    color=discord.Color.random())
            try:
    ...
    async def slowmode_error(self, ctx, error):
        ...
        if isinstance(error, commands.BadArgument):
            await ctx.send(
                embed=discord.Embed(title="How hard is it to set a slowmode :rolling_eyes: ",
