...
class WordGames(commands.Cog):
    ...
    @cog_ext.cog_slash(name="wordhunt", description="Test your skill in finding hidden words.")
    async def _wordhunt(self, ctx: SlashContext):
        ...
        try:
            ...
            if not points:
                await ctx.send("Looks like nobody could find a word. That's disappointing! :(\n\n"
    ...
    @cog_ext.cog_slash(name="extremehunt", description="A much more difficult version of wordhunt itself!")
    async def _extremehunt(self, ctx: SlashContext):
        long_words = sample(self.searchwords[6], randint(1, 2)) + sample(self.searchwords[randint(6, 8)], randint(1, 2))
        other_words = sample(self.searchwords[5], randint(2, 3)) + sample(self.searchwords[4], randint(2, 3))
        wordlist = long_words + other_words
        grid, skipped = self.generate_grid(wordlist)
    ...
    @cog_ext.cog_slash(name="scramble", description="Unscramble the word that I have scrambled for you.")
    async def _scramble(self, ctx: SlashContext):
        ...
        try:
            ...
            await ctx.channel.send(embed=nextcord.Embed(title="Looks like nobody knew the answer.",
                                                        description="The original word was **%s**." % word.upper(),
                                                        color=nextcord.Color.random()))
        if resp:
            await ctx.channel.send(embed=nextcord.Embed(title="**Congratulations** %s" % resp.author.name,
    ...
    @cog_ext.cog_slash(name="passthebomb",
                       description="Stop the bomb from exploding in your facing by giving me a suitable word.")
    async def _passthebomb(self, ctx: SlashContext):
        players = []
        ...
        def check(reaction, user):
            if not user.bot and user not in players and str(
        ...
        try:
            ...
            if len(players) < 2:
                ...
                return
        players = players[:6]
        two_letter = ['ab', 'ad', 'ag', 'am', 'an', 'ar', 'at', 'ay', 'by', 'do', 'en', 'es', 'et', 'fe', 'it',
                      'mo',
                      'ma', 'pe', 'py', 're', 'ti', 'us']
    ...
    @cog_ext.cog_slash(name="hangman", description="Save an innocent man by finding the letters in my hidden word!")
    async def _hangman(self, ctx: SlashContext):
        """
...
"""
    ...
    @cog_ext.cog_slash(name="typerace",
                       description="Let's you flex in front of your friends with your superior typing speed.")
    async def _typerace(self, ctx: SlashContext):
        players = []
        text = random.choice(open("text_files/random_sentences.txt", encoding="utf-8").readlines()).replace('\n', '')
        with open("text_files/random_sentences.txt") as myFile:
